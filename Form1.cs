...
namespace Remote
{
    public partial class Form1 : Form
    {
        ...
        string puttyPath = @".\putty.exe";   // Putty路径
        const string IniFileName = "config.ini"; // 默认账密配置文件
        const string RDPFileName = ".RDPconfig";  // RDP临时文件夹

        // 读取系统属性
        static bool deleteFlag = Properties.Settings.Default.deleteFlag;
        static bool tcpPingFlag = Properties.Settings.Default.tcpPingFlag;
        static bool fullScreenFlag = Properties.Settings.Default.fullScreenFlag;
        static bool topFlag = Properties.Settings.Default.topFlag;
        static bool SSHFlag = Properties.Settings.Default.SSHFlag;
        static string screenMode = "1";  // 全屏=0，窗口=1

        
        // private bool isVisible = true;
        // private const int MaxSize = 65536; // 最大缓冲区大小，适当调整以满足你的需求
        ...
        {
            // 同步单选框状态
            checkBox1.Checked = tcpPingFlag;
            checkBox2.Checked = fullScreenFlag;
            checkBox3.Checked = deleteFlag;
            checkBox4.Checked = topFlag;
            checkBox5.Checked = SSHFlag;
            TopMost = topFlag;
...
");
            ...
            flashFileList();
            LoadSectionsToComboBox(comboBox2, IniFileName);
        ...
        private async Task HandleTextBoxAsync(TextBox textBox, Label label, string defaultText = null)
        {
            if (!string.IsNullOrEmpty(textBox.Text))
            {
                ...
                if (checkBox5.Checked && defaultText == "SSH")
                {
        ...
        private async Task<bool> HandleTextBoxAsyncBySSH(TextBox textBox, Label label, bool PingFlag,string defaultText = null)
        {
            if (!string.IsNullOrEmpty(textBox.Text))
            {
                ...
                if (checkBox5.Checked && defaultText == "SSH" && PingFlag)
                {
                    int delay = await MeasureTcpPingExecutionTime(addr, timeoutMilliseconds: 1000);
                    if (delay < 500 && delay > 0)
                    {
                ...
                else if (!checkBox5.Checked || defaultText != "SSH")
                {
                    int delay = await MeasureTcpPingExecutionTime(addr, timeoutMilliseconds: 1000);
                    if (delay < 500 && delay > 0)
                    {
        ...
        {
            await HandleTextBoxAsync(textBox1, label4,"1");
            await HandleTextBoxAsync(textBox2, label5,"2");
            await HandleTextBoxAsync(textBox3, label6,"3");
            await HandleTextBoxAsync(textBox4, label7,"4");
            if(SSHFlag && checkBox6.Checked)
            {
                if (!lastPingSuccess1)
                {
                    lastPingSuccess1 = await HandleTextBoxAsyncBySSH(textBox6, label8, lastPingSuccess1, checkBox5.Checked ? "SSH" : "RDP");
                }
                if (!lastPingSuccess2) { 
                    lastPingSuccess2 = await HandleTextBoxAsyncBySSH(textBox7, label3, lastPingSuccess2, checkBox5.Checked ? "SSH" : "RDP"); 
                }

            }
            else
            {
                await HandleTextBoxAsync(textBox6, label8,  checkBox5.Checked ? "SSH" : "RDP");
                await HandleTextBoxAsync(textBox7, label3, checkBox5.Checked ? "SSH" : "RDP");
            }
        }
...
password {password}";
        ...
        {
            string puttyPath = "putty";  // 请替换为你的 PuTTY 安装路径
            string xshellPath = "xshell";  // 请替换为你的 XShell 安装路径

            // 拼接 PuTTY 和 XShell 命令行参数
            string puttyArgs = $"-ssh -l {username} -pw \"{password}\" -P {port} {ip}";
            string xshellArgs = $"ssh {username}:{password}@{ip}:{port}";

            // 创建一个 ProcessStartInfo 对象，用于设置启动 PuTTY 的参数
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show($"未找到Putty或Xshell", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                GenerateAndExecuteRdpFileAsync(textBox1);
            }

        }
        private void button3_Click(object sender, EventArgs e)
        {
            if (textBox2.Text.Length > 0) { GenerateAndExecuteRdpFileAsync(textBox2); }
        ...
        {
            if (textBox3.Text.Length > 0) { GenerateAndExecuteRdpFileAsync(textBox3); }

        }

        private void button5_Click(object sender, EventArgs e)
        {
            if (textBox4.Text.Length > 0) { GenerateAndExecuteRdpFileAsync(textBox4); }
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            HandleSSHLogin(textBox7);
        }

        private void button10_Click_2(object sender, EventArgs e)
        {

            HandleSSHLogin(textBox6);
        }
        ...
        {
            textBox5.Text = RemoveChineseAndTrim(textBox5.Text);
            string addr = "";
            string username = "";
            string password = "";
            ...
            int size = GetPrivateProfileString(comboBox2.Text, "端口", "", result, result.Capacity, iniFilePath);
            string port = result.ToString().Trim();
            addr = textBox5.Text + ":" + port;
            //Console.WriteLine(addr);

            result.Clear();
            size = GetPrivateProfileString(comboBox2.Text, "账号", "", result, result.Capacity, iniFilePath);
            username = result.ToString().Trim();
            //Console.WriteLine(username);

            result.Clear();
            size = GetPrivateProfileString(comboBox2.Text, "密码", "", result, result.Capacity, iniFilePath);
        ...
        private void checkBox2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            fullScreenFlag = checkBox2.Checked;
        }
        private void checkBox3_CheckedChanged(object sender, EventArgs e)
        {
            deleteFlag = checkBox3.Checked;
        }
        private void checkBox4_CheckedChanged(object sender, EventArgs e)
        {
            topFlag = checkBox4.Checked;
        }
        ...
        {
            SSHFlag = checkBox5.Checked;

            label3.Text = checkBox5.Checked ? "SSH" : "RDP";
            label8.Text = checkBox5.Checked ? "SSH" : "RDP";
        }
